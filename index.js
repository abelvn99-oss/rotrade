...
function calcOP(item) {
  if (!item.value) return { opGive:0, opGet:0 };
  if (item.projected) return { opGive:0, opGet:Math.round(item.value*0.15) };
  const pctMap = {
    Terrible:[0.12,0.02], Low:[0.08,0.03], Normal:[0.05,0.05],
    High:[0.03,0.08],     Amazing:[0.02,0.12],
  };
  const [gPct,tPct] = pctMap[item.demand] ?? [0.05,0.05];
  return { opGive:Math.round(item.value*gPct), opGet:Math.round(item.value*tPct) };
}
...
// ─── Best price from Roblox ─────────────────────────────────────────────────
async function fetchBestPrice(assetId) {
  try {
    const { data } = await axios.get(
      `https://economy.roblox.com/v1/assets/${assetId}/resellers?limit=5`,
      { timeout:8000, headers:{"User-Agent":"RoTradeBot/1.0"} }
    );
...
async function fetchDB() {
  ...
  for (const url of ENDPOINTS) {
    try {
      ...
      if (data.items && Object.keys(data.items).length>100) {
        raw=data.items;
        console.log(`[Rolimons] Loaded ${Object.keys(raw).length} raw items from ${url}`);
        break;
      }
  ...
  for (const [id,f] of Object.entries(raw)) {
    ...
    const value=Number(f[2]), rap=Number(f[10]);
    if (!value||value<=0) continue;
    parsed[id]={
      id, name:f[0]||"Unknown", acronym:f[1]||"",
      value, rap:(rap&&rap>0)?rap:null,
...
function buildItemEmbed(item, bestPrice, op, rsi) {
  ...
  const rsiStr = rsi===null?"`Insufficient data`"
    :rsi<30?`\`${rsi}\` — Oversold (below green zone — good time to trade for this)`
    :rsi<50?`\`${rsi}\` — Below midline (weak, may recover)`
    :rsi>70?`\`${rsi}\` — Overbought (above red zone — may drop soon)`
    :`\`${rsi}\` — Neutral (above midline)`;
  ...
  const demandNote={
    ...
    Amazing:"Exceptional demand. Actively sought after — expect large OP from interested parties.",
  };

  const flags=[
    item.projected?"🚨 Projected — RAP artificially inflated. Do not trade at face value.":null,
    item.rare?"💎 Rare":null, item.hyped?"🔥 Hyped":null,
  ].filter(Boolean);
  ...
  const embed=new EmbedBuilder()
    ...
    .setDescription([
      item.acronym?`**${item.acronym}**`:null,
      `[View on Rolimons](https://www.rolimons.com/item/${item.id})`,
    ].filter(Boolean).join("  ·  "))
    .addFields(
      {name:"Value",     value:`\`${fmt(item.value)} R$\``,                                    inline:true},
      {name:"RAP",       value:item.rap?`\`${fmt(item.rap)} R$\``:`\`Not tracked\``,          inline:true},
      {name:"Best Price",value:bestPrice?`\`${fmt(bestPrice)} R$\``:`\`Not available\``,      inline:true},
      {name:"vs RAP",    value:vsRap!==null?`\`${parseFloat(vsRap)>=0?"+":""}${vsRap}%\``:`\`N/A\``, inline:true},
      {name:"Demand",    value:item.demand?`\`${item.demand}\``:`\`Not set\``,                 inline:true},
      {name:"Trend",     value:item.trend?`\`${item.trend}\``:`\`Not set\``,                   inline:true},
      {
        name:"OP to Give",
        value:op.opGive>0
          ?`\`${fmt(op.opGive)} R$\`\nTypical overpay you would give when offering this item.`
          :`\`0 R$\` — Do not overpay for this item.`,
        inline:false,
      },
      {
        name:"OP to Get",
        value:op.opGet>0
          ?`\`${fmt(op.opGet)} R$\`\nTypical overpay to expect when someone wants this item.`
          :`\`0 R$\` — No OP expected.`,
        inline:false,
      },
      {name:"RSI (14-period)", value:rsiStr, inline:false},
    );

  if (item.demand&&demandNote[item.demand]) {
    embed.addFields({name:"Demand Insight", value:demandNote[item.demand]});
  }
  if (flags.length) embed.addFields({name:"Flags", value:flags.join("\n")});

  embed.setFooter({text:`Rolimons · ${new Date().toUTCString()}`});
