...
async function fetchAllItems() {
  ...
  const { data } = await axios.get("https://www.rolimons.com/itemapi/itemdetails", {
  ...
  for (const [id, f] of Object.entries(raw)) {
    ...
    const rap   = Number(f[10] ?? f[3] ?? 0);
    if (!value || value <= 0 || rap <= 0) continue;
    ...
    parsed[id] = {
      ...
      value,
      rap,
      demand:    parseDemand(f[4]),   // null if Rolimons hasn't set it
      trend:     parseTrend(f[5]),    // null if Rolimons hasn't set it
      projected: Number(f[6]) === 1,
      hyped:     Number(f[7]) === 1,
      rare:      Number(f[8]) === 1,
      bestPrice: Math.round(rap * 0.92),
      thumbUrl:  null,
    };
  }
